// =============================================================================
// File:           mobile/ios/QervonCustomerApp/AppSession.swift
// Project:        Qervon
...
public final class AppSession: ObservableObject {
    ...
    public init() {
        let store = KeychainTokenStore(service: "com.qervon.ios.customer")
        tokenStore = store
    ...
    public func unlockWithBiometrics() async -> Bool {
        let success = await biometricGate.unlock(
            reason: "Qervon oturumunuzu açmak için kimliğinizi doğrulayın"
        )
        if success {
            state = .active
        }
